...
export const SellClient = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase.from("brands").select("*").eq("category_id", categoryRow.id).order("name");
      const items = (data || []) as CatalogItem[];
      setBrands(items);
      if (route[1]) setBrand(items.find((item) => item.slug === route[1]) || null);
    })();
  }, [category]);
  ...
  useEffect(() => {
    if (!brand) return;
    (async () => {
      const { data } = await supabase.from("series").select("*").eq("brand_id", brand.id).order("name");
      const items = (data || []) as CatalogItem[];
      setSeries(items);
      if (route[2]) setSelectedSeries(items.find((item) => item.slug === route[2]) || null);
    })();
  }, [brand]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase.from("models").select("*").eq("series_id", selectedSeries.id).eq("active", true).order("name");
      const items = (data || []) as CatalogItem[];
      setModels(items);
      if (route[3]) setModel(items.find((item) => item.slug === route[3]) || null);
    })();
  }, [selectedSeries]);
