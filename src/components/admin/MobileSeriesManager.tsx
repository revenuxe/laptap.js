...
export function MobileSeriesManager() {
  ...
  const { data: series, isLoading } = useQuery({
    queryKey: ["mobile-series", mobileCategory?.id],
    queryFn: async () => {
      if (!mobileCategory) return [];
      const { data, error } = await supabase
        .from("series")
        .select("*, brands!inner(name, category_id)")
        .eq("brands.category_id", mobileCategory.id)
        .order("name");
      if (error) throw error;
      return data;
    },
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mobile-series"] });
      toast.success("Mobile series created successfully");
      resetForm();
      setOpen(false);
    },
    onError: (error: any) => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mobile-series"] });
      toast.success("Mobile series updated successfully");
      resetForm();
      setOpen(false);
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mobile-series"] });
      toast.success("Mobile series deleted successfully");
    },
    onError: (error: any) => {
      toast.error(error.message);
    },
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Mobile Series</CardTitle>
        <Dialog open={open} onOpenChange={(open) => { setOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingSeries ? "Edit Mobile Series" : "Add New Mobile Series"}</DialogTitle>
            </DialogHeader>
