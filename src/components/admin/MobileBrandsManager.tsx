...
export function MobileBrandsManager() {
  ...
  useEffect(() => {
    if (mobileCategory) {
      setCategoryId(mobileCategory.id);
    }
  }, [mobileCategory]);
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase
        .from("brands")
        .insert({ name, country, category_id: categoryId, logo_url: logoUrl })
        .select()
        .single();
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("brands")
        .update({ name, country, category_id: categoryId, logo_url: finalLogoUrl })
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Mobile Brands</CardTitle>
        <Dialog open={open} onOpenChange={(open) => { setOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingBrand ? "Edit Mobile Brand" : "Add New Mobile Brand"}</DialogTitle>
            </DialogHeader>
